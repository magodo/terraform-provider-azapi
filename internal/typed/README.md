...
- Simple lifecycle operations: Each resource shall follow the same and simple lifecycle operation patterns, i.e. `PUT` for Create, `GET` for Read, `GET`-then-`PUT` for Update, `DELETE` for delete. Only for some edge cases (e.g. a post-write polling is needed for mitigating ARM eventual consistency issue), we shall not introduce additional lifecycle API calls. This also means we shall avoid introducing *artificial* properties at the resource type level to control a resource's runtime behavior.
